...
PY
  ...
  echo "PHASE 13/16 artifact audit"
  python3 - "$OUTPUT/campaign.json" "$code_commit" <<'PY'
import json,sys
data=json.load(open(sys.argv[1]))
assert data["implementation_commit"] == sys.argv[2]
assert data["policies"] == ["J100","J060","J040","J020"]
assert len(data["scenarios"]) == 4
for scenario in data["scenarios"]:
    assert scenario["candidate_fault_count"] >= scenario["recoverable_fault_count"]
print("EXP10 PASS")
PY
  echo "PHASE 14-16/16 deterministic artifacts, results analysis and disk audit are recorded in campaign.json"
}
