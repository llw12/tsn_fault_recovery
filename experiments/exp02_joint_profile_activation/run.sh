...
OUTPUT_ROOT="$PROJECT_ROOT/results/joint_profile_activation"
...
inside_environment() {
    ...
    run_scenario joint "$PROJECT_ROOT/simulations/recovery_joint_profile" JointProfileRecovery "$raw_root"
    python3 "$PROJECT_ROOT/scripts/analyze_joint_experiment.py" --mode joint --raw-root "$raw_root" --output-dir "$run_output"
    mkdir -p "$OUTPUT_ROOT"
    cp "$run_output"/{summary.csv,joint_profile_tt_packets.csv,summary.md,tt_timeline.png,gcl_activation_log.csv} "$OUTPUT_ROOT/"
    local previous
    previous="$(find "$OUTPUT_ROOT/runs" -mindepth 1 -maxdepth 1 -type d ! -name "$run_id" | sort | tail -n 1)"
    if [[ -n "$previous" ]]; then
        python3 "$PROJECT_ROOT/scripts/compare_experiment_runs.py" --first "$previous/summary.csv" --second "$run_output/summary.csv" --output "$OUTPUT_ROOT/reproducibility.csv"
    fi
}
...
echo "Completed exp02 run $RUN_ID"
