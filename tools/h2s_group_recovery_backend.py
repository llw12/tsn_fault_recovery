"""Multi-physical-fault H2S/CELF backend used only by exp21."""
...
def fault_group_id(disabled_links: tuple[str, ...] | list[str]) -> str:
    return "FG_" + hashlib.sha256(canonical_json_bytes(sorted(set(disabled_links)))).hexdigest()


class H2sGroupRecoveryBackend(H2sJrsBackend):
    """ALL_REROUTE synthesis for a group of two or more simultaneous faults."""
    name = "h2s-group-recovery"
    ...
    def __init__(self, executable: Path, *, quantum_ns: int = DEFAULT_QUANTUM_NS,
                 ...
                 h2s_flow_sorting: int = 4, attempt_celf_fallback: bool = True):
        super().__init__(executable, quantum_ns=quantum_ns, candidate_paths=candidate_paths,
                         memory_limit_mb=memory_limit_mb, h2s_tiebreak_mode=h2s_tiebreak_mode,
                         h2s_tiebreak_seed=h2s_tiebreak_seed, h2s_flow_sorting=h2s_flow_sorting,
                         attempt_celf_fallback=attempt_celf_fallback, diagnostic_trace_path=None)

    def synthesize(self, request: RecoverySynthesisRequest) -> RecoverySynthesisResult:
        started = time.perf_counter_ns()
        if request.forwarding_model != "stream-aware" or request.route_scope != "all-reroute":
            return RecoverySynthesisResult(self.name, BackendStatus.UNSUPPORTED,
                diagnostic="group recovery requires stream-aware ALL_REROUTE")
        faults = tuple(sorted(set(request.disabled_links)))
        if len(faults) < 2 or not request.healthy_primary_routes:
            return RecoverySynthesisResult(self.name, BackendStatus.INVALID_INPUT,
                diagnostic="group recovery requires at least two physical faults and healthy P0 routes")
        try:
        ...
        flows = {str(flow["id"]): flow for flow in scenario.get("tt_flows", [])}
        ...
        disconnected = [flow_id for flow_id in sorted(flows)
                        if str(flows[flow_id]["destination"]) not in reachable_nodes(
                            scenario, set(faults), str(flows[flow_id]["source"]))]
        if disconnected:
            return RecoverySynthesisResult(self.name, BackendStatus.STRUCTURAL_NO_ROUTE,
                statistics={"structurally_disconnected_flows": disconnected, "semantic_valid": False},
                timings_ms={"total_backend": (time.perf_counter_ns() - started) / 1e6},
                diagnostic="group topology disconnects at least one TT flow")
        output = request.output_directory or request.scenario_path.parent / "h2s_group_input"
        ...
        try:
            prepared = prepare_h2s_inputs(request.scenario_path, output, self.quantum_ns, self.candidate_paths,
                disabled_links=faults, healthy_primary_routes=request.healthy_primary_routes,
                affected_flow_ids=tuple(sorted(affected)), route_scope="all-reroute")
        ...
        attempts, resource_statuses, invalid = [], [], False
        for algorithm in (("H2S", "CELF") if self.attempt_celf_fallback else ("H2S",)):
            ...
            if run_status is not None:
                resource_statuses.append(run_status); invalid |= run_status == BackendStatus.OUTPUT_INVALID; continue
            ...
            try:
                ...
                verification_started = time.perf_counter_ns(); checker = check_h2s_pf_solution(prepared, normalized)
            ...
            if all_scheduled and raw["upstream_verifier_pass"] and checker["valid"]:
                ...
                group_id = fault_group_id(faults); profile = normalized["profile"]
                profile.update({"profile_id": f"PFG_{group_id[3:19]}", "fault_group_id": group_id,
                    "fault_group_ids": list(faults), "disabled_physical_links": list(faults),
                    "affected_flow_ids": sorted(affected), "route_scope": "all-reroute", "backend": self.name})
                ...
                return RecoverySynthesisResult(self.name, status, feasible=True,
                    ...
                    timings_ms={"conversion": conversion_ms,
                        ...
                        "h2s_wall": attempts[0]["wall_ms"], "celf_wall": attempts[1]["wall_ms"] if len(attempts) > 1 else 0,
                        "total_backend": (time.perf_counter_ns() - started) / 1e6})
            invalid |= all_scheduled and not checker["valid"]
        if BackendStatus.MEMORY_LIMIT in resource_statuses: status = BackendStatus.MEMORY_LIMIT
        elif BackendStatus.TIME_LIMIT in resource_statuses: status = BackendStatus.TIME_LIMIT
        elif invalid: status = BackendStatus.OUTPUT_INVALID
        elif attempts and all(a.get("requested_flow_count") and a.get("scheduled_flow_count", 0) == 0 for a in attempts):
            status = BackendStatus.CANDIDATE_ROUTE_FAILURE
        else: status = BackendStatus.HEURISTIC_NOT_FOUND
        ...
        return RecoverySynthesisResult(self.name, status,
            ...
            diagnostic="H2S and CELF did not produce an all-flow static-valid group schedule")
